"""Streaming, split-aware, multi-worker iteration over a corpus of Designs.
...
from synbiotorch.distributed import env_rank_world, worker_shard
from synbiotorch.encoders.base import SupportsEncode
...
def iter_split_records(
    ...
    if callable(iter_for_worker):
        records: Iterable[Design] = iter_for_worker(global_id, global_count)
    else:
...
def _shuffle_buffered(stream: Iterator[Design], buffer_size: int, seed: int) -> Iterator[Design]:
